...
프로젝트 코드를 Qdrant에 인덱싱하여 의미론적 검색을 수행합니다.
...
| `PostToolUseFailure` | 도구 사용 실패 시 실행 |
| `Notification` | 알림 발생 시 실행 |
| `UserPromptSubmit` | 사용자 프롬프트 제출 시 실행 |
| `SessionStart` | 세션 시작 시 실행 |
| `SessionEnd` | 세션 종료 시 실행 |
| `Stop` | Claude Code 중지 시 실행 |
| `SubagentStart` | 서브에이전트 시작 시 실행 |
| `SubagentStop` | 서브에이전트 종료 시 실행 |
| `PreCompact` | 컨텍스트 압축 전 실행 |
| `PermissionRequest` | 권한 요청 시 실행 |
| `Setup` | 초기 설정 시 실행 |
...
**원격 브랜치 삭제:** 리모트 브랜치도 대시보드에서 삭제 가능 (보호 브랜치 제외)
...
Claude 세션 파일 기반 실시간 분석 대시보드입니다.

**데이터 소스**: `~/.claude/projects/` 세션 파일에서 직접 수집 (DB 설정과 무관)
...
**트렌드 차트** (시간 범위: 1h, 24h, 7d, 30d, all):
- 세션 수 추이 (`created_at` 기준 버킷팅)
- 토큰 사용량 / 비용 추이
- 성공률 추이

**모델별 성능**: 실제 모델명(claude-opus-4-6, claude-haiku 등)으로 그룹화

**비용 분석**: 프로젝트별 + 모델별 이중 분류, 월간 비용 예측

**활동 히트맵**: 요일/시간대별 세션 시작 패턴

**멀티 프로젝트 비교**: 최대 5개 프로젝트의 메트릭 비교 차트
...
*이 가이드는 Agent Orchestration System 기준으로 작성되었습니다. (최종 업데이트: 2026-03-02)*
