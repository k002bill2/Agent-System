...
- 구성: 규칙 13,048(16.2%) + 메모리 인덱스 8,733(10.8%) + **나머지 58,816(73.0%)** = 시스템 프롬프트 · 도구 스키마 · MCP 서버 지침 · 스킬 목록 · output style
- 즉 **CLAUDE.md 계열이 직접 통제하는 표면은 13,048(16.2%)**뿐이다

> **2026-08-17 재정정:** 이 분해에 한때 `@.env.example` 주입 1,961(2.4%)을 별도 항목으로 넣고 CLAUDE.md 계열을 18.6%로 적었으나, **그 주입은 애초에 일어나지 않았다.** `.env` 계열 파일은 CLAUDE.md 의 `@` 확장에서 차단된다(아래 함정 ⓔ 실측). 위 수치가 정정본이다.
...
> **⚠ 실행 디렉토리를 격리할 것.** 측정 시점의 페이로드에는 프로젝트 CLAUDE.md의 `@.env.example` 한 줄이 들어 있었고, **`@path` 는 stdin 프롬프트에서 확장된다**(함정 ⓓ·ⓔ — CLAUDE.md 경로에서는 `.env` 계열이 차단되지만 **stdin 경로에서는 확장된다**). 레포 루트에서 실행하면 실제 `.env.example` 이 붙어 규칙 토큰이 부풀고 재현이 깨진다. 아래처럼 **참조 대상 파일이 존재하지 않는 작업 디렉토리**를 만들어 거기서 측정한다(원 측정도 그렇게 했다).
...
- ⓓ **`@path` 확장은 CLAUDE.md 안에서만 일어나는 게 아니다 — stdin 프롬프트에서도 확장된다.** 실측(61,550자 파일): 프롬프트에 `@big.txt`를 넣으면 42,638 → 62,390 토큰(**+19,752**). 따라서 페이로드 측정은 참조 대상 파일이 **없는** 디렉토리에서 해야 한다(원 측정도 그러했다).
- ⓔ **`@` 확장은 경로에 따라 `.env` 계열을 다르게 다룬다 — CLAUDE.md 에서는 차단, stdin 에서는 허용.** 동일 61,550자 파일을 이름만 바꿔 4조건 실측:

  | 조건 | 첫 턴 토큰 | 대조군(백틱) 대비 | 판정 |
  |---|---|---|---|
  | CLAUDE.md 에 `@big.txt` (일반) | 59,889 | +17,251 | 확장 |
  | CLAUDE.md 에 `` `big.txt` `` (대조군) | 42,638 | — | — |
  | CLAUDE.md 에 `@.big.txt` (히든) | 60,328 | +17,690 | 확장 |
  | **CLAUDE.md 에 `@.env.big`** | **42,178** | **-460** | **차단** |
  | stdin 프롬프트에 `@.env.big` | 62,391 | +20,213 | 확장 |

  히든 파일이라서가 아니라 **`.env` 이름 패턴** 때문이다. 설계 의도가 읽힌다 — 사용자가 직접 타이핑한 `@`는 명시적 요청이라 허용하고, CLAUDE.md 의 자동 주입은 시크릿 유출 경로라 막는다.

  **이 발견이 뒤집은 것:** 앞서 "`@.env.example` 이 1,961 토큰 상시 주입 중"이라고 적었던 근거는 **stdin 경로 실측(1,798)** 이었고, 그것을 CLAUDE.md 경로에도 적용한 것이 오류였다. 프로젝트 CLAUDE.md 의 `@.env.example` 은 **전환 전에도 주입되지 않았다**. 백틱 전환(후속과제 ⑤)의 토큰 이득은 **0**이며, 남는 가치는 (a) 읽는 사람의 오해 제거, (b) 파일명이 `.env` 패턴을 벗어나도록 바뀌면 그때부터 실제로 주입되는 잠복 위험 차단이다.

  > **일반화:** `@` 로 참조했다고 주입을 가정하지 말 것. 주입 여부는 **파일명 패턴과 참조 위치**에 달렸고, 확인은 위 방식(고신호 파일로 대조군 대비 차분)으로 실측해야 한다. 반대로 8,850자짜리 파일로 재려 하면 신호(~2k)가 세션 노이즈(실측 ±1,400, 동일 조건 3회 범위)에 묻혀 판별되지 않는다.

**후속 과제:** ① APFS/LiveMetro/Universal-Environment-Setup의 install.sh 사본은 별개 해시로 이미 드리프트 — 이번 동기화 범위 밖(Agent-System 쌍만 동기화) ② ~~새 세션 시작 후 베이스라인 토큰 실측으로 최종 확인~~ → **2026-08-17 완료** (위 절) ③ 새 이력은 CLAUDE.md가 아니라 harness-changelog.md(하네스)·HISTORY.md(글로벌 규칙)에 기입 ④ **다음 다이어트의 큰 표적은 CLAUDE.md 밖에 있다** — 베이스라인의 **73.0%**가 도구 스키마·MCP 지침·스킬 목록이고 CLAUDE.md 계열은 **16.2%**다. ~~미사용 MCP 서버·플러그인 정리가 자릿수 큰 이득이다~~ → **2026-08-17 정정**: MCP 도구 스키마는 `ToolSearch` 뒤로 deferred라 상주하지 않는다(실측 `192 tools · 0 tokens`) — **서버 정리의 토큰 이득은 0**이며, 이유는 정리(유지·인증할 연결 감소)에 그친다. 그 73.0% 안의 실제 레버는 **스킬 목록**(정리 후 실측 8.6k)과 **메모리**(23k, 이 중 인덱스 4.9k·리콜된 파일 18k)다 ⑤ ~~그 18.6% 안에 값싼 한 줄이 있다: 프로젝트 CLAUDE.md 18행 `@.env.example`은 상시 주입 중이며 8,850자 = 1,961 토큰 — 한 글자 지우기로 회수할 수 있다.~~ → **2026-08-17 완료(단, 전제가 틀렸음)**: `CLAUDE.md`·`AGENTS.md` 18행을 백틱 경로(`` `.env.example` ``)로 전환했으나, **토큰 이득은 0이다** — `.env` 계열은 CLAUDE.md 의 `@` 확장에서 애초에 차단되므로 전환 전에도 주입되지 않았다(함정 ⓔ 4조건 실측). 전환의 남은 가치는 오해 제거와 잠복 위험 차단이며, 1,961은 "이 파일을 통째로 넣으면 드는 비용"일 뿐 실제 주입량이 아니었다.
