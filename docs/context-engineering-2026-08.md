...
**후속 과제:** ① APFS/LiveMetro/Universal-Environment-Setup의 install.sh 사본은 별개 해시로 이미 드리프트 — 이번 동기화 범위 밖(Agent-System 쌍만 동기화) ② ~~새 세션 시작 후 베이스라인 토큰 실측으로 최종 확인~~ → **2026-08-17 완료** (위 절) ③ 새 이력은 CLAUDE.md가 아니라 harness-changelog.md(하네스)·HISTORY.md(글로벌 규칙)에 기입 ④ **다음 다이어트의 큰 표적은 CLAUDE.md 밖에 있다** — 베이스라인의 70.5%가 도구 스키마·MCP 지침·스킬 목록이고 CLAUDE.md 계열은 18.6%다. ~~미사용 MCP 서버·플러그인 정리가 자릿수 큰 이득이다~~ → **2026-08-17 정정**: MCP 도구 스키마는 `ToolSearch` 뒤로 deferred라 상주하지 않는다(실측 `192 tools · 0 tokens`) — **서버 정리의 토큰 이득은 0**이며, 이유는 정리(유지·인증할 연결 감소)에 그친다. 그 70.5% 안의 실제 레버는 **스킬 목록**(정리 후 실측 8.6k)과 **메모리**(23k, 이 중 인덱스 4.9k·리콜된 파일 18k)다 ⑤ 다만 그 18.6% 안에 값싼 한 줄이 있다: 프로젝트 CLAUDE.md 18행 `@.env.example`은 상시 주입 중이며 **8,850자 = 1,961 토큰(실측)** — 규칙 전체(13,048)의 15%를 한 글자 지우기로 회수할 수 있다. ~~백틱 경로 전환을 검토할 것.~~ → **2026-08-17 완료**: `CLAUDE.md`·`AGENTS.md` 18행을 백틱 경로(`` `.env.example` ``)로 전환. 위 1,961 토큰은 **전환 전 스냅샷 기록**이며 현재는 주입되지 않는다.
