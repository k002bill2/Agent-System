...
| **arity 검사** (Task 8 승격, `8385c1a` 로 순서 대조 추가) | 스텁 선언 파라미터 = 전달 인자 = impl 파라미터 — **개수와 이름 순서** 양쪽. 커버리지 무관 전수 | 스텁과 impl 이 **함께** 원본과 다른 순서로 바뀐 경우, 기본값 |
...
>
> **2026-08-08 3차 정정 — 위 표의 arity 행은 절반만 낡았다.** `8385c1a` 가 impl 파라미터
> **이름 순서 대조**를 추가해, 위 표의 둘째 줄 중 **impl 쪽만 뒤바뀐 경우는 이제 잡힌다**
> (Red-Green 실증: 순서 뒤바꿈 주입 → FAIL, 수정 전 `arity.ts` 로는 2 passed 무음).
> 여전히 안 잡히는 것은 **스텁과 impl 이 함께** 원본과 다른 순서로 옮겨진 경우다 — 둘이
> 서로 일치하므로 현재-시제 검사로는 원리적으로 보이지 않고, 1회성 원본 대조만이 잡는다.
> 이 문단을 "arity 가 순서를 못 잡는다" 로 통째로 읽지 마라. 그것 자체가 새로운 거짓 주장이다.
>
