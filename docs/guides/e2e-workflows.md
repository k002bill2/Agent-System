...
6. [ACE 프레임워크 통합](#workflow-6-ace-프레임워크-통합)
...
## Workflow 6: ACE 프레임워크 통합

AOS의 기존 `ace-framework` 스킬과 연동하는 워크플로우.

### ACE 4-Pillar 모델

```
┌─────────────────────────────────────────┐
│ Pillar 1: Soft Constraints (프롬프트)    │
│ → CLAUDE.md, 스킬, 에이전트 프롬프트     │
├─────────────────────────────────────────┤
│ Pillar 2: Hard Constraints (훅)         │
│ → PreToolUse 차단, 파일 보호, 검증       │
├─────────────────────────────────────────┤
│ Pillar 3: Boundaries (격리)             │
│ → 워크트리, 파일 소유권, 도구 제한        │
├─────────────────────────────────────────┤
│ Pillar 4: Optimization (분석)           │
│ → 메트릭 수집, 피드백 루프, 개선           │
└─────────────────────────────────────────┘
```
...
### 병렬 실행 (parallel-coordinator)

3개 이상 독립 작업 시 `parallel-coordinator` 스킬 사용:

```
Feature 구현 요청
    ↓
[parallel-coordinator] 작업 분해
    ↓
┌──────────────┬──────────────┬──────────────┐
│ Agent 1      │ Agent 2      │ Agent 3      │
│ React UI     │ FastAPI API  │ Tests        │
└──────┬───────┴──────┬───────┴──────┬───────┘
       └──────────────┼──────────────┘
                      ↓
              [quality-validator] 검증
                      ↓
              /verify-app → /commit-push-pr
```
