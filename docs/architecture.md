...
| sweep 은 **전체를 페이지로 순회**한다 (기본키 커서) | 같은 함수의 while 루프 | 한 페이지만 보면 앞을 차지한 살아 있는 세션들 뒤의 만료 세션이 영영 안 걸림 |
| 커서는 **시각 컬럼이 아니다** | `repo.list_metadata_for_sweep` | `created_at`·`updated_at` 은 nullable — NULL 이 커서에 들어가면 행 값 비교가 unknown 이 되어 이후 페이지가 통째로 비고 sweep 이 조용히 멈춤 |
...
두 인스턴스가 **동시에** 갱신·삭제를 시도하는 크로스 프로세스 경합은 조건부 문장으로
...
한 페이지로 묶이지만 한 번의 sweep 비용은 세션 수에 비례한다.

커서는 **기본키(`id`)** 다. 시간순으로 방문할 이유가 없다 — 전체를 순회하므로 어느
순서로 보든 모두 걸린다. 커서에 필요한 성질은 고유·불변·NOT NULL·인덱스뿐이고
기본키가 전부 만족한다. 시각 컬럼은 두 가지 이유로 쓰지 않는다: `created_at`·
`updated_at` 이 **nullable** 이라 NULL 이 커서에 들어가면 이후 페이지가 통째로 비고,
`updated_at` 은 거기에 더해 시계가 둘이다(issue #309).
...
init_db()
  ...
  └─ _run_migrations()            # 기존 배포 따라잡기 (information_schema 조회 후 raw DDL)
       └─ reconcile_schema()      # db/schema_reconcile.py — 컬럼·인덱스·외래키 수렴
```
...
`create_all()` 은 **기존 테이블에 컬럼을 추가하지 않는다.** 그 간극은 리콘실러
(`src/backend/db/schema_reconcile.py`)가 메타데이터와 `information_schema` 를
대조해 자동으로 메운다. 컬럼 → 인덱스 → 외래키 순서로 세 패스를 돌며,
뒤 패스는 앞 패스가 무엇을 건너뛰었는지 보고 그 위에 얹지 않는다:
...
| **FK 제약** 복구 | ⚠️ 조건부 | **고아 행을 먼저 세고 0 일 때만** 붙인다. 0 이 아니면 붙이지 않고 건수를 보고한다 |
| DROP · 타입 변경 | ❌ | 파괴적이라 사람이 판단할 일 |

FK 가 조건부인 이유: 무조건 `ADD CONSTRAINT` 는 고아 행이 하나만 있어도 검증에 실패해
**기동을 통째로 막고**, `NOT VALID` 는 더러운 배포의 쓰기 경로를 예고 없이 런타임
에러로 바꾼다. 반면 리콘실러가 방금 채운 컬럼은 전부 NULL 이라 고아가 정의상 0 이므로,
조건부 규칙은 **리콘실러가 만드는 케이스를 전량 수렴시키면서** 기존 더러운 데이터에는
손대지 않는다 (issue #330). `ADD FOREIGN KEY` 에는 `IF NOT EXISTS` 가 없고 모델의 FK 는
전부 익명이라, 중복 방지는 `(테이블, 컬럼, 참조 테이블)` 탐지 키가 전담한다 — 이름으로
맞추면 전부 "없음" 으로 오판해 기동마다 제약이 쌓인다.
