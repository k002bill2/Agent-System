...
class TestPruneMergedEndpoint:
    ...
    async def test_dry_run_returns_candidates_no_delete(self, client):
        ...
        with (
            patch("api.git._legacy.get_git_service_for_project", return_value=git_service),
            patch("api.git._legacy.get_github_service", return_value=MagicMock()),
            patch("api.git._legacy._get_db_session", new=AsyncMock(return_value=None)),
        ):
    ...
    async def test_no_github_token_returns_503(self, client):
        ...
        with (
            patch("api.git._legacy.get_git_service_for_project", return_value=git_service),
            patch("api.git._legacy.get_github_service", side_effect=raise_503),
            patch("api.git._legacy._get_db_session", new=AsyncMock(return_value=None)),
        ):
            resp = await client.post(PRUNE_PATH, json={"dry_run": True})
