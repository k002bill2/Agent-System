...
`nodes` 는 RAG·MCP 를 `try/except ImportError` 로 감싸 의존이 없어도 동작하게
만든다. 문제는 **`except ImportError` 가 어떤 ImportError 든 삼킨다**는 것이다 —
모듈 분할이 순환 import 를 만들면 그것도 ImportError 로 잡혀
...
표면을 `route_table.py` 가 지켰지만 `nodes` 에는 라우트가 없다 —
...
`MCPToolExecutor=orchestrator.tools.MCPToolExecutor`. 분할 후 같은 값이어야 한다.

참조 경로는 분할에 맞춰 서브모듈로 옮겼다 — 패키지 `__init__` 은 노드 클래스
6종만 재노출한다(좁은 재노출). 여기서 `orchestrator.nodes.RAG_AVAILABLE` 을
계속 읽으려고 `__init__` 에 별칭을 추가하면, 그 별칭은 플래그가 실제로 쓰이는
`planner`/`executor` 의 전역과 다른 객체가 되어 이 테스트가 눈먼다.
"""

import importlib

from orchestrator.nodes import executor, planner
...
def test_rag_flag_matches_actual_importability() -> None:
    ...
    assert planner.RAG_AVAILABLE is True, (
    ...
    assert planner.get_project_context.__module__ == "services.rag_service", (
        f"get_project_context 가 fallback 이다 (모듈={planner.get_project_context.__module__}). "
