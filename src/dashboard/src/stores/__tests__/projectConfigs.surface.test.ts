/**
 * projectConfigs 스토어 분할이 공개 표면을 바꾸지 않았음을 보증한다.
 *
 ...
 *   it('gen', () => {
 *     writeFileSync('src/stores/__tests__/projectConfigs.surface.json',
 *       JSON.stringify(surface(useProjectConfigsStore.getState() as Record<string, unknown>), null, 2) + '\n')
 *   })
 *
 ...
 *
 * 한계: `surface()` 는 JSON 왕복이라 `Set`/`Map` 초기값이 `{}` 로 평탄화된다 — 이 스토어의
 * `Set` 7개가 베이스라인에 `{}` 로 굳어 있어 그 내용이 바뀌어도 스냅샷은 통과한다.
 *
...
import { useProjectConfigsStore } from '../projectConfigs'
import baseline from './projectConfigs.surface.json'
import { surface } from './storeSurface'

describe('projectConfigs 스토어 표면', () => {
  it('상태 키·액션 이름·초기값이 분할 전과 같다', () => {
    expect(surface(useProjectConfigsStore.getState() as Record<string, unknown>)).toEqual(baseline)
  })
})
