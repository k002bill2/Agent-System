 ...
 *
 * 스토어 구성:
 * - authStore: 인증 상태 (로그인, 사용자 정보, 역할)
 * - agentStore: 에이전트 레지스트리 (목록, 상태, 통계)
 * - taskStore: 태스크 관리 (CRUD, 상태 추적, 통계)
 * - uiStore: UI 상태 (테마, 모달, 토스트, 사이드바)
 */

// Domain Stores (슬라이스 패턴)
export { useAuthStore, selectIsAdmin, selectUserRole } from './authStore'
export type { AuthUser, UserRole } from './authStore'

export { useAgentStore, selectAvailableAgents, selectAgentById, selectAgentsByCategory } from './agentStore'
export type { AgentInfo, AgentCapability, AgentCategory, AgentStatus, AgentRegistryStats } from './agentStore'
