...
export type PendingStatus = 'queued' | 'retrying' | 'failed'
...
export interface PendingTaskEvaluation {
  id: string
...
interface FeedbackState {
  ...
  pendingEvaluations: PendingTaskEvaluation[]
    ...
    (set, get) => ({
      ...
      submitFeedback: async (feedback: FeedbackSubmit, agentId?: string) => {
        ...
        try {
          ...
          set((state) => ({
            pendingFeedbacks: [...state.pendingFeedbacks, pendingItem],
            error: message,
            isSubmitting: false,
          }))
          return null
      ...
      submitTaskEvaluation: async (evaluation: TaskEvaluationSubmit) => {
        ...
        try {
          ...
          const pendingItem: PendingTaskEvaluation = {
            id: crypto.randomUUID(),
            evaluation,
            status: 'queued',
            retryCount: 0,
          ...
          set((state) => ({
            pendingEvaluations: [...state.pendingEvaluations, pendingItem],
            error: message,
            isSubmitting: false,
          }))
          return null
      ...
      retryPendingSubmissions: async () => {
        const { pendingFeedbacks, pendingEvaluations } = get()

        // Retry pending feedbacks
        for (const item of pendingFeedbacks) {
          if (item.retryCount >= item.maxRetries) {
            set((state) => ({
              pendingFeedbacks: state.pendingFeedbacks.map((p) =>
                p.id === item.id ? { ...p, status: 'failed' as PendingStatus } : p
              ),
            }))
            continue
          }
          ...
          set((state) => ({
            pendingFeedbacks: state.pendingFeedbacks.map((p) =>
              p.id === item.id ? { ...p, status: 'retrying' as PendingStatus } : p
            ),
          }))
          ...
          try {
            ...
            set((state) => ({
              pendingFeedbacks: state.pendingFeedbacks.map((p) =>
                p.id === item.id
                  ? { ...p, status: 'queued' as PendingStatus, retryCount: p.retryCount + 1, lastError: message }
                  : p
              ),
            }))
          }
        ...
        for (const item of pendingEvaluations) {
          if (item.retryCount >= item.maxRetries) {
            set((state) => ({
              pendingEvaluations: state.pendingEvaluations.map((p) =>
                p.id === item.id ? { ...p, status: 'failed' as PendingStatus } : p
              ),
            }))
            continue
          ...
          try {
            ...
            set((state) => ({
              taskEvaluations: { ...state.taskEvaluations, [key]: result },
              pendingEvaluations: state.pendingEvaluations.filter((p) => p.id !== item.id),
            }))
          } catch (error) {
            const message = error instanceof Error ? error.message : 'Retry failed'
            set((state) => ({
      ...
      clearPending: (id: string) => {
        set((state) => ({
          pendingFeedbacks: state.pendingFeedbacks.filter((p) => p.id !== id),
          pendingEvaluations: state.pendingEvaluations.filter((p) => p.id !== id),
        }))
      },

      clearAllPending: () => {
        set({ pendingFeedbacks: [], pendingEvaluations: [] })
      },
    }),
    {
      name: 'feedback-storage',
      partialize: (state) => ({
        pendingFeedbacks: state.pendingFeedbacks,
...
if (typeof window !== 'undefined') {
  window.addEventListener('focus', () => {
    const { pendingFeedbacks, pendingEvaluations, retryPendingSubmissions } = useFeedbackStore.getState()
    if (pendingFeedbacks.length > 0 || pendingEvaluations.length > 0) {
