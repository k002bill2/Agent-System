...
class MCPService:
    ...
    async def _execute_tool(self, name: str, arguments: dict[str, Any]) -> MCPToolResult:
        ...
        try:
            ...
            return MCPToolResult(
                content=[{"type": "text", "text": f"Tool execution error: {str(e)}"}], isError=True
            )
...
To track progress, use aos_get_status with session_id: {session_id}"""
        ...
        return MCPToolResult(content=[{"type": "text", "text": result_text}])
    ...
    async def _tool_get_status(self, args: dict[str, Any]) -> MCPToolResult:
        ...
        if session_id not in self._session_tasks:
            return MCPToolResult(
                content=[{"type": "text", "text": f"Session not found: {session_id}"}], isError=True
            )

        task_info = self._session_tasks[session_id]
        return MCPToolResult(content=[{"type": "text", "text": json.dumps(task_info, indent=2)}])
    ...
    async def _tool_list_agents(self, args: dict[str, Any]) -> MCPToolResult:
        ...
        agents = {
            "development": [
                {"name": "web-ui-specialist", "model": "sonnet", "description": "Web UI/UX expert"},
                {
        ...
        return MCPToolResult(content=[{"type": "text", "text": json.dumps(result, indent=2)}])
    ...
    async def _tool_run_check(self, args: dict[str, Any]) -> MCPToolResult:
        ...
        return MCPToolResult(
            content=[{"type": "text", "text": "\n".join(output_lines)}], isError=not all_success
        )
    ...
    async def generate_sse_stream(self) -> AsyncIterator[str]:
        ...
        # Send endpoint info
        yield self.create_sse_event("endpoint", {"url": "/mcp/messages"})
