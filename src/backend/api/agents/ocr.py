"""ocr 관련 Agents API 라우트.

Task Analyzer 용 이미지 텍스트 추출(OCR). 비전 모델 런타임 해석과
LLM 사용량 기록 헬퍼가 함께 산다 — 전부 이 핸들러 전용이다.

`__init__.py` 가 `_resolve_ocr_runtime` 과 `extract_text_from_image` 를
재노출한다(패키지 밖 테스트가 각각 import·모듈 속성으로 가져간다).
"""
