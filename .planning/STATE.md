...
## Current Position
**2026-08-28 기준 — main `7cf54e5`. 열린 PR 0 건. provider 중립 세션 트랙의 잔여 항목이 전부 닫혔고,
DB 모드 posture 도 결정·머지됐다(#336).**

| 항목 | 상태 |
|---|---|
| 열린 PR | **0 건** (#339 `df238fd` · #340 `68d09a2` · #341 `02e79dc` · #342 `e82485d` · #343 `0ec6b28` · #344 `87ba018` 전부 squash 머지. 이 중 #339 · #342 · #344 세 건은 머지 후 파일 단위 내용 대조로 안착을 확인했다 — squash 는 "PR 이 닫혔다" 와 "내용이 main 에 들어갔다" 를 다르게 보장한다) |
| 열린 이슈 | **#345** 세션 권한 회수 — 활동 스토어 회수와 요청 세대 경쟁. #343 의 Codex 리뷰가 9 라운드에 걸쳐 제기했으나 그 PR 범위 밖이라 사유와 함께 분리한 2 건이다. 표시 결함이 아니라 별도 기능의 요구사항 |
| 사용자 결정 대기 | **없음.** DB 모드 posture 는 2026-08-27 **차단 해제**로 결정돼 **#336 `5b03970`** 로 머지됐다. 이 행은 #346 이 Current Position 을 갱신할 때 병행 중이던 #336 을 보지 못해 남아 있었다. 실측: `access.py` 에 `unavailable in database mode` 0 건, 해당 3 지점이 인가 통과로 바뀐 상태 |
| 잔여 작업 (착수 가능) | **파일 분할 B5.5·B6** 하나뿐이다 — 800 줄 초과 백엔드 15 개 (2026-08-28 재실측, 변동 없음. 최대 `organization_service.py` 2,216 · 이하 `claude_session_monitor.py` 1,866 · `git_service.py` 1,756) |
| 백로그(이슈 미등록) | `get_session_activity` 가 전사와 같은 결함을 갖는다 — offset 이 레코드가 아니라 필터된 이벤트를 세므로 #344 의 레코드 윈도우로는 못 푼다. 별도 계산 문제 · `scan.truncated` 가 전사 응답까지 배선돼 있지 않다 — 레코드 10 만·스캔 256MiB 중 먼저 닿는 쪽에서 총계가 프리픽스가 되는데 절단 사실이 응답에 안 실린다(아래 #344 항목) |
...
- **후속(미결) 2 의 조사 결과 — 2026-08-28. 우려가 사실로 확인됐고, 범위는 더 넓다.**
  ...
  **2 단계 완료 (2026-08-28) — 프로젝트 단위 인가.** `api/git/_shared.py` 의
  `require_git_project_access` 가 라우터 게이트를 대체한다(인증 + 인가).

  **ID 해석 통일은 "통일"이 아니라 우회로 풀었다.** 두 ID 공간을 변환하는 대신 **양쪽이
  공유하는 필드인 파일시스템 경로로 조인**한다 — 레지스트리 `Project.path` ↔
  `ProjectModel.path` → UUID → `require_project_role`. git 라우트는 어차피 레지스트리에서
  Project 를 꺼내 그 `path` 를 쓰므로 추가 조회가 아니다.

  - 읽기 viewer / 그 외 editor (`project_configs/access.py` 의 filesystem 분기와 같은 규칙).
  - DB 모드에서 **DB 미등록 경로는 404** — 레지스트리에 있다는 사실만으로는 접근권이 없다.
    `7ed7c46` 이 연 바로 그 틈이다.
  - `project_id` 없는 `/github/...` 계열은 인증만. 인가할 프로젝트가 없다.
  - `require_project_role` 의 "ACL 행이 없으면 editor" 하위호환 정책을 그대로 따르므로
    **오늘의 배포에서 동작 변화는 없다.** ACL 행이 생기는 시점부터 실제로 막힌다.

  회귀 4 건 추가(총 7 건). 판별은 Red-Green 이 아니라 **뮤테이션**으로 했다 — 게이트를 통째로
  지우면 전부 실패해 어느 단언이 무엇을 잠그는지 알 수 없기 때문이다:
  - ID 변환을 되돌리면(UUID 대신 레지스트리 키) → 해당 테스트 **1 건만** 실패
  - 쓰기 권한 구분을 없애면(전부 viewer) → 해당 테스트 **1 건만** 실패

  `test_api_git_prune.py` 픽스처는 `get_current_user` 대신 게이트 자체를 오버라이드하도록
  바꿨다 — 그 파일의 `proj-1` 은 실제 레지스트리 항목이 아니라 대역이다.

  **후속 1(project-config 쓰기 권한 기준이 모드별로 갈린다)은 아직 열려 있다.** 같은 뿌리로
  묶어 두었으나 이번 작업은 git 쪽만 닫았다. project-config 의 DB 분기는 여전히
  `ProjectAccess` 행의 존재만 보고 role 을 보지 않는다 — 같은 path-조인 방식을 적용할 수 있다.
