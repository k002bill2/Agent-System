...
- **후속(미결) 2 의 조사 결과 — 2026-08-28. 우려가 사실로 확인됐고, 범위는 더 넓다.**
  ...
  **수정 방향 (2 단계)**:
  1. `api/git/__init__.py` 의 라우터에 `dependencies=[Depends(get_current_user)]` — 미인증 차단.
     라우터 소유 모듈에 두면 다른 곳에서 include 해도 따라간다. `commits.py` 의 optional 사용은
     draft-commit 이 비로그인 흐름을 의도한 것인지 확인이 필요하다.
  2. 프로젝트 단위 인가. **후속 1 과 뿌리가 같다** — 라우트 `project_id` 는 path 기반 문자열이고
