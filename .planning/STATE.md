...
| 잔여 작업 (착수 가능) | **B6 — 집중도 70% 이상 8 개.** 전부 `playground_service` 와 같은 성질(메서드 추출)이라 `lift_staticmethods.py` 를 그대로 쓴다. 목록·주의는 아래 "2026-08-28 (이어서 4)" |
...
**2 단계가 실제로 무엇이었는지 — B6 8 개가 전부 이 성질이다.**
