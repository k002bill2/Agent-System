...
## Current Position
**2026-08-28 기준 — main `31c8318`. provider 중립 세션 트랙 종료, 파일 분할 B5.5 완주 (5/5).**
...
| 잔여 작업 (착수 가능) | **B6 — 800 줄 초과 잔여 9 개**(실측 2026-08-28. 계획서의 "8 개" 는 실측과 다르다). 전부 `playground_service` 와 같은 성질(메서드 추출)이라 `lift_staticmethods.py` 를 그대로 쓴다. 목록·주의는 아래 "2026-08-28 (이어서 4)" |
...
- **800줄 초과 파일** — 프로그램 진행 중. **B1~B5.5 완료**, 백엔드 잔여 **10개**(실측 2026-08-28,
  B5.5 다섯 개 머지 후). 그중 `api/v1/agent_registry.py` 는 제외 결정이므로 **B6 대상은 9개**다
  (계획서의 "8개" 도, 이 줄에 있던 "15개" 도 실측과 다르다 — 매번 다시 잰다).
  상위 계획서: `docs/plans/2026-08-04-oversized-file-split.md`. 테스트 파일은 한도 제외(사용자 결정 2026-08-04).
  잔여 10개는 전부 집중도 48% 이상(B6 대상)이거나 `api/v1/agent_registry.py`(프로덕션
...
### 2026-08-28 (이어서 4) — B5.5 완주(5/5). 다음은 B6

**#360 `31c8318`** — `playground_service.py` 1,286 줄 → 5 모듈, 최대 738 줄.
B5.5 다섯 개가 전부 한도 안에 들어왔다:

| 대상 | 원본 | 최대 모듈 | PR |
|---|---:|---:|---|
| audit_service | 986 | 633 | #349 `3e07203` |
| tmux_service | 920 | 618 | #354 `16fca73` |
| merge_service | 1,328 | 707 | #356 `1746335` |
| notification_service | 1,017 | 753 | #358 `a43219f` |
| playground_service | 1,286 | 738 | #360 `31c8318` |

**2 단계가 실제로 무엇이었는지 — B6 9 개가 전부 이 성질이다.**

`PlaygroundService` 가 혼자 808 줄이라 정의 이동만으로는 못 들어갔다. 그런데
**클래스 전체가 `@staticmethod`** 였다 — 즉 상태를 가진 클래스가 아니라
네임스페이스다. 그러면 다음이 성립한다:

    def f(x): ...                     # 모듈 레벨, 본문 바이트 그대로
    class C:
        f = staticmethod(f)           # `C.f(x)` 호출 형태가 완전히 보존된다

**mypy 가 이 형태를 그대로 통과시킨다**(실측). 대안이던 위임 함수(`@staticmethod
def f(...): return mod.f(...)`)는 메서드당 5 줄이라 `service.py` 를 790 줄까지
밀어올렸을 것이다. 도구로 남겼다 — **`tests/backend/api/lift_staticmethods.py`**.
`@staticmethod` 가 아니면 중단하고(self/cls 를 쓰므로 들어올릴 수 없다), 들어올린
뒤에도 원 클래스를 참조하면 중단한다(그대로 두면 두 모듈이 서로를 import 한다).

**어느 그룹을 들어올릴지는 크기가 아니라 제약으로 정한다.** DB 그룹을 고른 것은
129 줄이라서가 아니라 `_initialized` 재바인딩 두 곳(`_load_sessions` ·
`load_sessions_from_db`)이 그것으로 한 모듈에 모이기 때문이다 — 그 전에는
48 + 808 = 856 줄이 묶여 있었다. **크기 문제와 상태 제약을 한 번에 푸는 절단면을
찾는 것이 2 단계의 설계 작업이다.**

**`global` 문만 보고 판정하면 틀린다 — AST 로 재바인딩과 변형을 구분할 것.**
`global _sessions, _initialized` 가 두 곳에 있었지만 `_sessions` 는 첨자 대입·삭제만
되고 **한 번도 재바인딩되지 않았다**(그 `global` 선언은 잉여다 — 첨자 대입에는
필요 없다). 그래서 여러 모듈이 같은 dict 를 보고 배럴 재노출도 안전했다. 진짜
제약은 `_initialized` 하나였다. `global` 을 그대로 믿었으면 불필요한 제약을 걸어
분할선을 못 찾았을 것이다. 판정 스크립트는 커밋 메시지 참조(`ast.Assign` 의 target
이 `Name` 이면 REBIND, `Subscript` 면 mutate).

**패치 타깃 방향이 앞의 셋과 반대인 경우가 있다.**
Task 2·3·4 는 정의와 읽는 쪽이 같은 모듈이라 그 모듈을 패치하면 됐다. Task 5 는
`_load_sessions` 등의 **정의가 `storage`, 호출 지점 21 곳이 전부 `service`** 였다.
`from .storage import _load_sessions` 는 바인딩을 **복사**하므로 패치는 정의처가
아니라 **읽는 쪽** 을 겨냥해야 한다(`playground_service.service`). `mock.patch`
문서의 "patch where it's used, not where it's defined" 그대로다.
→ **배정표를 쓰기 전에 "정의가 어디로 가나" 가 아니라 "호출 지점이 어디로 가나"
를 범위별로 세라.** 하나라도 정의처 쪽에 남으면 그 이름은 reader 가 갈린 것이다.

**감사는 두 단계로 나눈다.** 1 단계 산출물 대비는 순수 이동이라 28 종 전부 통과하고,
원본 대비는 1 단계 변경분(신규 5 종 + 클래스 본문 1 건)만 드러난다. 한 번에
감사했으면 **무엇이 이동이고 무엇이 편집인지 구분할 수 없었다.**

**깊이 결합 경로는 예고대로 있었다** — `STORAGE_DIR`. #358 의 `DATA_DIR` 과 같은
처리(본문 수정 + 독립 앵커 단언 + 되돌림 RED).

---

**다음 — B6 (800 줄 초과 잔여 9 개 — 실측. 계획서는 "70% 이상 8 개" 로 적었다).**
계획서는 "파일 분할이 아니라 메서드 추출·설계 재검토 성질" 이라 적었고 그 판정은
맞다. Task 5 에서 그 성질의 도구와 절차가 확립됐으므로 그대로 쓴다:

1. `lift_staticmethods.py` 로 1 단계 → 2. `split_*.py` 배정표로 2 단계 →
3. 두 단계 감사 → 4. 전후 실행 비교 → 5. 계약 테스트 + Red-Green

**B6 대상 9 개 (실측 2026-08-28, B5.5 머지 후):**
`organization_service` 2,216 · `claude_session_monitor` 1,866 · `git_service` 1,756 ·
`rag_service` 1,534 · `analytics_service` 1,522 · `feedback_service` 1,375 ·
`project_config_monitor` 1,293 · `llm_service` 874 · `cost_allocation_service` 812.
(`api/v1/agent_registry.py` 816 은 제외 결정. 계획서의 "8 개" 와 이전 STATE 의 "15 개" 는
둘 다 실측과 다르다 — **매번 다시 잰다.**)

**주의 — `rag_service`(1,534)는 테스트가 `rag_mod` 별칭으로 모듈 객체 패치를
20 건 이상 쓴다.** Task 5 의 12 건보다 많고, 그중 reader 가 갈리는 것이 있으면
배정표가 그것에 끌려간다. 착수 전 "호출 지점 범위별 카운트" 를 먼저 할 것.
클래스가 `@staticmethod` 가 아니면 `lift_staticmethods.py` 가 중단하므로,
그때는 진짜 설계 재검토(인스턴스 메서드 분리)가 필요하다 — 다른 성질의 일이다.
...
| ~~playground_service~~ | ~~1,286~~ | 63% | 963 | 0 | 12 | **완료 (#360 `31c8318`)** — 2 단계 |
