...
2. 실행 모드 결정 — **위에서부터 순서대로 평가하고, 처음 일치하는 분기 하나만 적용한다**:
   - **미존재** → 초기 실행. Phase 1로
   - **[최우선] 존재 + Phase A가 BLOCKED** (사유 불문) → Phase A 관문이 아직 닫혀 있으므로 **아래 어떤 분기보다 먼저** 이 경로에서 처리한다. 범위 지정이 섞여 있어도(예: "승인하되 백엔드만 다시") 부분 재실행이나 새 실행으로 내려보내지 않는다. RUN_STATE의 BLOCKED 사유로 하위 분기:
     - **사유=승인 대기** + `A_planner_plan.md` 존재 + **원안 그대로 승인** → **승인 재개**. `_workspace/` 이동·계획 재생성 없이 저장된 계획을 채택하고 Phase B부터 이어간다
     - **사유=승인 대기** + **수정·범위 변경이 동반된 승인** → **Phase A 재실행**(planner 재호출 → 계획 갱신 → 승인 절차 반복). 변경된 범위가 승인 없이 Phase B에 도달하지 않게 한다
     - **사유=planner 실행 실패**(2회 실패 — `A_planner_plan.md` 부재 가능) → 입력 내용과 무관하게 **Phase A 재실행**. 승인 입력이 와도 재개가 아니다 — 실패를 승인으로 우회할 수 없다
...
- **승인 재개 규칙**(BLOCKED는 정상 경로이므로 출구를 정의한다): Phase A가 **승인 대기 사유로** BLOCKED이고 `A_planner_plan.md`가 존재하는 상태에서 뒤늦게 사용자 승인이 오면 — (**planner 2회 실패로 인한 BLOCKED는 이 규칙의 대상이 아니다** — 승인 입력이 와도 Phase A를 재실행한다. 실패를 승인으로 우회할 수 없다) — (a) **원안 승인** → RUN_STATE의 Phase A를 `DONE`으로 갱신하고(승인 시각·근거 발화 기록) 저장된 `A_planner_plan.md`를 **그대로** 채택해 Phase B부터 이어간다. `_workspace/`를 새 실행으로 이동하거나 planner를 다시 부르지 않는다. (b) **수정 요청과 함께 온 승인** → planner를 재호출해 계획을 갱신하고(`A_planner_plan.md` 덮어쓰기) 승인 절차를 다시 거친다. 어느 경우든 승인 없이 Phase B로 넘어가지 않는다
