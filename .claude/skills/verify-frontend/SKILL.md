...
non-zero 종료는 차단 신호가 아니라 발견 신호다 — 리포트는 정상 생성된 것이므로 Bash 도구가 오류로 표시해도 위 표와 아래 '예외' 목록을 적용해 실제 위반 여부를 판정하라. 휴리스틱(grep) 검사 특성상 예외 해당 항목(동적 스타일, 멀티라인 시그니처 등)이 카운트에 포함될 수 있다.
